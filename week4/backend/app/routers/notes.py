...
def search_notes(q: str | None = None, db: Session = Depends(get_db)) -> list[NoteRead]:
    if not q:
        ...
        # SQLite's LIKE is only case-insensitive for ASCII; lower() both sides
        # to make the search reliably case-insensitive.
        needle = f"%{q.lower()}%"
        rows = (
            db.execute(
                select(Note).where(
                    (func.lower(Note.title).like(needle)) | (func.lower(Note.content).like(needle))
                )
            )
            .scalars()
            .all()
